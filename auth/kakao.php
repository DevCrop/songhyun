...
if ($email !== '') {
    ...
    if ($existingByEmail) {
		...
		echo "<script>
			alert('이미 동일한 이메일로 가입된 계정이 있습니다. 일반 로그인 후 카카오 연동을 진행해 주세요.');
...
// ------------------------------------------------------
// 6) 리다이렉트 (state 파싱 + 토큰검증 + 안전한 path만 허용)
// ------------------------------------------------------
$redirectPath = '/'; // 기본값
...
if (isset($_GET['state'])) {
    ...
    if (json_last_error() === JSON_ERROR_NONE && is_array($stateArr)) {
        // CSRF 토큰 검증
        $sessionToken = $_SESSION['kakao_oauth_state_token'] ?? '';
        $stateToken   = $stateArr['token'] ?? '';

        if ($sessionToken && $stateToken && hash_equals($sessionToken, $stateToken)) {
            // path 후보 추출
            $candidate = (string)($stateArr['path'] ?? '/');

            // 절대 URL, 외부 도메인, 프로토콜 스킴 금지
            if (preg_match('#^(https?:)?//#i', $candidate)) {
                $candidate = '/';
            }

            // 내부 경로만 허용 (원하면 여기 정규식/화이트리스트를 프로젝트 규칙에 맞게 강화)
