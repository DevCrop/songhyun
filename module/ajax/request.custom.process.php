...
// $feces_time       = $_POST['feces_time'] ?? null; // 사용 안 함
// $urine_time       = $_POST['urine_time'] ?? null; // 사용 안 함
$feces_days          = $_POST['feces_days'] ?? null;
$feces_count         = $_POST['feces_count'] ?? null;
$urine_days          = $_POST['urine_days'] ?? null;
$urine_count         = $_POST['urine_count'] ?? null;
...
try {
    ...
    $query = "
        INSERT INTO nb_custom_inquires (
            ...
            feces_days, feces_count, urine_days, urine_count,
    ...
    $stmt->bindValue(':weight', (int)$weight, PDO::PARAM_INT);
    $stmt->bindValue(':consult_time', (int)$consult_time, PDO::PARAM_INT);
    $stmt->bindValue(':first_visit', (int)$first_visit, PDO::PARAM_INT);
    if ($branch_id !== null && $branch_id !== '') {
        $stmt->bindValue(':branch_id', (int)$branch_id, PDO::PARAM_INT);
    } else {
        $stmt->bindValue(':branch_id', null, PDO::PARAM_NULL);
    }
    if ($feces_days !== null && $feces_days !== '') $stmt->bindValue(':feces_days', (int)$feces_days, PDO::PARAM_INT);
    if ($feces_count !== null && $feces_count !== '') $stmt->bindValue(':feces_count', (int)$feces_count, PDO::PARAM_INT);
    if ($urine_days !== null && $urine_days !== '') $stmt->bindValue(':urine_days', (int)$urine_days, PDO::PARAM_INT);
    if ($urine_count !== null && $urine_count !== '') $stmt->bindValue(':urine_count', (int)$urine_count, PDO::PARAM_INT);
